...
{
	PD_ODR &= ~(1<<5); // âûðóáàåì äåìóëüòèïëåêñîð
	PC_ODR = (cnt<<3); // çàïèñûâàåì â äåìóëüòèïëåêñîð íîâîå çíà÷åíèå
	PD_ODR |= (1<<5); // âêëþ÷àåì äåìóëüòèïëåêñîð
	
	TIM2_SR1 = 0; // ñáðàñûâàåì ôëàã Update Interrupt Pending
	
	//TIM2_CR1 |= 1; // âêëþ÷àåì òàéìåð
		
	cnt++; 
	cnt &= 7; // äâèãàåì ñ÷åò÷èê LED
	
	TIM2_CCR1L = ~colors[cnt][0]; // ïåðåäàåì â áóôåð çíà÷åíèÿ
	TIM2_CCR2L = ~colors[cnt][1]; // äëÿ ñëåäóþùåãî öèêëà ØÈÌ
